...
def predict_fitness():
    ...
    if (
        userHeight == 51
    ):  # Just a fix because python is removing trailing zeros while coverting to str or int
        userHeight = 510
    ...
    classifier = classifier.fit(
        features, labels
    )  # Find common patterns in training data
    ...
    is_Weight_In_between = (
        userWeight >= fromEpectedWeight and userWeight <= toExpectedWeight
    )
    ...
    fitData = {"isFit": is_Weight_In_between, "message": message}
